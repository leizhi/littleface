...
<body>
<form id="default" name="default" method="Post" action="<%=request.getContextPath()%>/NoteType.do" >
...
<!-- SearchTable -->
	...
	<li class="Input"><html:input type="text" id="Category" name="Category" property="Category" size="15" maxlength="255"/></li>
        ...
        <li class="Input"></li>
     	...
     	<li class="Button"><input type="button" id="Find" name="Find" value="Find" onclick="Util.Submit('/NoteType.do?state=list');"/><input type="button" id="Add" name="Add" value="Add" onclick="Util.Submit('/NoteType.do?state=promptAdd');"/><input type="reset" id="reset" name="reset" value="reset"/></li>
      </ul>
    </div>
...
<logic:iterate property="List">
	...
	<li><%=Input.getValue("Category"+i)%></li>
        ...
        <li><a href="javascript:Util.Submit('/NoteType.do?state=promptUpdate&Key=<%=Input.getValue("ID"+i)%>');"><html:message key="Update" /></a> <a href="javascript:Util.Submit('/NoteType.do?state=processDelete&Key=<%=Input.getValue("ID"+i)%>');"><html:message key="Delete" /></a></li>
<% i++; %>
</logic:iterate>
