...
function JSDragDropTree() {
	...
	this.imageFolder = 'jsp/images/';
	this.folderImage = 'dhtmlgoodies_folder.gif';
	this.fileImage = 'dhtmlgoodies_sheet.gif';
...
JSDragDropTree.prototype = {
	...
	removeEvent : function(whichObject, eventType, functionName) {
		if (whichObject.detachEvent) {
			whichObject.detachEvent('on' + eventType, whichObject[eventType
					+ functionName]);
	...
	setFileImage : function(imagePath) {
	...
	expandAll : function() {
		var menuItems = document.getElementById(this.idOfTree)
	...
	moveDragableNodes : function(e) {
		...
		if (thisObj && thisObj.id) {
			...
			if (this.tagName == 'A' || JSTreeObj.dragNode_noSiblings) {
				...
				tmpObj.style.left = (JSTreeObj.getLeftPos(eventSourceObj) + JSTreeObj.indicator_offsetX_sub)
						+ 'px';
			} else {
				tmpImg.src = tmpImg.src.replace('ind2', 'ind1');
				JSTreeObj.insertAsSub = false;
				tmpObj.style.left = (JSTreeObj.getLeftPos(eventSourceObj) + JSTreeObj.indicator_offsetX)
						+ 'px';
			}

			tmpObj.style.top = (JSTreeObj.getTopPos(thisObj) + JSTreeObj.indicator_offsetY)
					+ 'px';
		}
	...
	dropDragableNodes : function() {
		...
		if (JSTreeObj.dragNode_destination) {
			if (JSTreeObj.insertAsSub) {
				var uls = JSTreeObj.dragNode_destination
				...
				var img = JSTreeObj.dragNode_destination
						.getElementsByTagName('IMG')[0];
				img.style.visibility = 'visible';
				img.src = img.src.replace(JSTreeObj.plusImage,
						JSTreeObj.minusImage);

			} else {
				if (JSTreeObj.dragNode_destination.nextSibling) {
					var nextSib = JSTreeObj.dragNode_destination.nextSibling;
					nextSib.parentNode.insertBefore(JSTreeObj.dragNode_source,
							nextSib);
				} else {
					JSTreeObj.dragNode_destination.parentNode
							.appendChild(JSTreeObj.dragNode_source);
				}
			}
			/* Clear parent object */
	...
	dragDropCountLevels : function(obj, direction, stopAtObject) {
		...
		if (direction == 'down') {
			...
			for ( var no = 0; no < subObjects.length; no++) {
				countLevels = Math.max(countLevels, JSTreeObj
						.dragDropCountLevels(subObjects[no], "up", obj));
			}
			return countLevels;
		}
	},
			...
			while (li) {
				if (li.id) {
					...
					if (numericID.length == 0)
						numericID = 'A';
					var numericParentID = li.parentNode.parentNode.id.replace(
							/[^0-9]/gi, '');
					if (numericID != '0') {
	...
	__deleteItem_step2 : function(obj) {
		...
		JSTreeObj.__addAdditionalRequestParameters(
				JSTreeObj.ajaxObjects[ajaxIndex],
	...
	__deleteComplete : function(ajaxIndex, obj) {
		if (this.ajaxObjects[ajaxIndex].response != 'OK') {
			alert('ERROR WHEN TRYING TO DELETE NODE: '
					+ this.ajaxObjects[ajaxIndex].response); // Rename failed
		} else {
	...
	__renameComplete : function(ajaxIndex) {
		if (this.ajaxObjects[ajaxIndex].response != 'OK') {
			alert('ERROR WHEN TRYING TO RENAME NODE: '
					+ this.ajaxObjects[ajaxIndex].response); // Rename failed
		}
	},
	__saveTextBoxChanges : function(e, inputObj) {
		...
		if (inputObj.value.length > 0) {
			...
			JSTreeObj.ajaxObjects[ajaxIndex] = new sack();
			JSTreeObj.ajaxObjects[ajaxIndex].method = "GET";
			JSTreeObj.ajaxObjects[ajaxIndex].setVar("renameId",
					inputObj.parentNode.id.replace(/[^0-9]/gi, ''));
			JSTreeObj.ajaxObjects[ajaxIndex].setVar("newName", inputObj.value);
			JSTreeObj.__addAdditionalRequestParameters(
					JSTreeObj.ajaxObjects[ajaxIndex],
					JSTreeObj.additionalRenameRequestParameters);
	...
	initTree : function() {
		...
		if (this.deleteAllowed || this.renameAllowed) {
			try {
				...
				if (this.deleteAllowed)
					menuModel.addItem(1, '删除', '', '', false,'JSTreeObj.deleteItem');
				if (this.renameAllowed)
					menuModel.addItem(2, '重命名', '', '', false,'JSTreeObj.renameItem');
				menuModel.init();

				var menuModelRenameOnly = new DHTMLGoodies_menuModel();
				if (this.renameAllowed)
					menuModelRenameOnly.addItem(3, '重命名', '', '', false,'JSTreeObj.renameItem');
				menuModelRenameOnly.init();

				var menuModelDeleteOnly = new DHTMLGoodies_menuModel();
				if (this.deleteAllowed)
					menuModelDeleteOnly.addItem(4, '删除', '', '', false,'JSTreeObj.deleteItem');
		...
		for ( var no = 0; no < menuItems.length; no++) {
			var isFolder = true;
			var tmpVar = menuItems[no].getAttribute('isFolder');
			if (!tmpVar)
				tmpVar = menuItems[no].isFolder;
			if (tmpVar == 'false')
				isFolder = false;
			// No children var set ?
			...
			if (menuItems[no].className) {
				...
				if (isFolder){
					...
					folderImg.src = this.imageFolder + this.fileImage;
				}
			}
