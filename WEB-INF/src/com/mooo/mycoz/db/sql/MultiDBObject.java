...
public class MultiDBObject  extends CommonMultiSQL implements MultiDbobj {
	...
	public List searchAndRetrieveList() {
		...
		try {
					...
					if (cacheManager.getInstance().isCacheEnabled()){
						//if ((mobject=(DBObjectList)cacheManager.get(CACHE_TYPE,querySQL)) != null)
					...
					//cacheManager.add(CACHE_TYPE,querySQL,new DBObjectList(resultList));
					}// enabled
		} catch (SQLException sqle) {
			System.err.println("Exception in DbSeesion:" + sqle);
			sqle.printStackTrace();
		...
		try {
				...
				//	for (int i = 1; i <= n; i++) {					
								//map.put(rs.getMetaData().getCatalogName(i), rs.getObject(i));
					//			System.err.println("column:" + rs.getMetaData().getColumnName(i));
					//			System.err.println("Lable:" + rs.getMetaData().getColumnLabel(i));
